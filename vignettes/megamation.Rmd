...
`mm_get()` gets data by using these credentials.
...
`mm_get()` uses five main functions: 

- `mm_req()` to create a request.
- `mm_req_params()` to filter by parameters.
...
`mm_req()` prints the request without performing it:

```{r}
req <- mm_req("workorder")
req
```

### Parameterize the Request
```{r}
req <- mm_req("workorder") |> mm_req_params(date = jan_2023)
req
```
